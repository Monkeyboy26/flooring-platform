...
const BASE_URL = 'https://www.rcglobalflooring.com';
const USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const MAX_ERRORS = 30;

/**
 * RC Global enrichment scraper for Tri-West.
 *
 * Scrapes rcglobalflooring.com for product images, descriptions, and specs.
...
/**
 * Attempts to find product on RC Global site.
...
async function findProductOnSite(page, group, delayMs) {
  try {
    const searchTerms = [
      group.name,
      group.collection.replace(/^RC\s+Global\s*/i, '').trim(),
    ...
    try {
      ...
      // Look for product/flooring links
      const productLinks = await page.$$eval('a[href*="product"], a[href*="floor"], a[href*="collection"], a[href*="catalog"]', links =>
...
async function extractProductData(page, productName) {
  try {
    ...
    const description = await page.evaluate(() => {
      const selectors = ['.description', '.product-description', 'p', 'article', '.content p'];
