...
const BASE_URL = 'https://hardwoods-inc.com';
const USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const MAX_ERRORS = 30;

/**
 * Hardwoods Specialty enrichment scraper for Tri-West.
 *
 * Scrapes hardwoods-inc.com for product images, descriptions, and specs.
 * Enriches EXISTING Tri-West SKUs — never creates new products.
 * Tech: Unknown (distributor site)
 *
...
export async function run(pool, job, source) {
  ...
  const brandPrefix = 'Hardwoods Specialty';
...
/**
 * Attempts to find product on Hardwoods Specialty site.
...
async function findProductOnSite(page, group, delayMs) {
  try {
    const searchTerms = [
      group.name,
      group.collection.replace(/^Hardwoods\s+Specialty\s*/i, '').trim(),
      group.skus[0]?.variant_name
    ].filter(Boolean);

    // Strategy 1: Navigate to products/catalog section
    try {
      await page.goto(BASE_URL, { waitUntil: 'networkidle2', timeout: 10000 });
      await delay(1000);

      // Look for products, catalog, or category links
      const catalogLinks = await page.$$eval('a[href*="product"], a[href*="catalog"], a[href*="laminate"], a[href*="veneer"], a[href*="plywood"]', links =>
        links.map(a => ({ text: a.textContent.trim(), href: a.href }))
      );

      if (catalogLinks.length > 0) {
        for (const term of searchTerms) {
          const match = catalogLinks.find(link =>
        ...
        // Try first catalog link as fallback
        if (catalogLinks.length > 0) {
          await page.goto(catalogLinks[0].href, { waitUntil: 'networkidle2', timeout: 10000 });
          await delay(1000);
    ...
    // Strategy 2: Try search if available
    try {
...
async function extractProductData(page, productName) {
  try {
    ...
    const description = await page.evaluate(() => {
      const selectors = ['.product-description', '.description', 'p', 'article p', '.content p', '.product-details p'];
    ...
    try {
      ...
      // Laminate/veneer/plywood specific specs
      ...
      const widthMatch = specText.match(/width[:\s]+([0-9/.]+\s*(?:mm|inch|in|ft|'))/i);
      ...
      const gradeMatch = specText.match(/grade[:\s]+([A-Z0-9\s]+)/i);
      if (gradeMatch) specs.grade = gradeMatch[1].trim();
