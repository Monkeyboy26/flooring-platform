    ...
    for (const color of lineColors) {
      ...
      if (ediData) {
        ...
        if (ediData.cost || ediData.retail_price) {
          ...
          // ~2x-of-cost retail to 1.70x cost so rescrapes don't revert the reprice.
          if (cost > 0 && retail > 0 && retail / cost >= 1.95 && retail / cost <= 2.05) retail = Math.round(cost * 1.70 / 0.05) * 0.05;
          await pool.query(`
