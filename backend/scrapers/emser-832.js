...
// Must match slugs used by emser-catalog.js (leaf categories: porcelain-tile,
// ceramic-tile, natural-stone, mosaic-tile, lvp-plank, trim-accessories, …)
const CATEGORY_MAP = {
...
const MAC_CATEGORY_MAP = {
  ...
  VINTILR: 'lvp-plank',
  VINTILC: 'lvp-plank',
  VINMISR: 'transitions-moldings',  // vinyl misc = Emcore moldings/adhesive
  SETMTL:  'adhesives-sealants',
  GRTCAU:  'grout',
  TRMACC:  'trim-accessories',
...
// installation-sundries / hardware-specialty buckets into leaves.
//
// This now DELEGATES to the central classifier (lib/categoryClassifier.js), which
// merged and superseded this scraper's original copy along with MSI's. Contract
// is unchanged: returns a leaf slug on a confident keyword match, else null (so
// the caller can leave the product on its parent for the choke-point net to
// best-guess + flag). Kept as a named export for callers that import it.
export function classifyEmserSundry(name) {
