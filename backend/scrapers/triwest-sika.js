...
/**
 * Sika enrichment scraper for Tri-West.
 *
 * Scrapes usa.sika.com for product images, descriptions, and specs.
...
/**
 * Find product on Sika AEM site and extract data
 */
async function findProductOnSite(page, group, delayMs) {
  ...
  // Try direct URL first: /en/construction/floor-covering/{product-name}.html
  const productUrl = `${BASE_URL}/en/construction/floor-covering/${slug}.html`;
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      // Extract description
      const descElement = document.querySelector('.product-description, .product-intro, [class*="description"]');
      if (descElement) {
        result.description = descElement.textContent.trim();
      }

      // Extract images from Scene7 CDN
      const imgElements = document.querySelectorAll('img[src*="sika.scene7.com"], picture source[srcset*="sika.scene7.com"]');
      const imageUrls = new Set();

      imgElements.forEach(img => {
        let url = img.src || img.srcset?.split(',')[0]?.split(' ')[0];
        if (url && url.includes('sika.scene7.com')) {
          // Remove query params and use high-res version
          url = url.split('?')[0];
          imageUrls.add(url);
        }
      });
      ...
      const techSections = document.querySelectorAll('[class*="technical"], [class*="spec"], [class*="property"]');
      techSections.forEach(section => {
        ...
        const potLifeMatch = text.match(/pot life[:\s]+([0-9]+)\s*(?:min|minutes)/i);
        if (potLifeMatch) {
          result.specs.pot_life = potLifeMatch[1] + ' minutes';
        }
        ...
        // Color
        const colorMatch = text.match(/color[:\s]+([a-z\s]+)/i);
        if (colorMatch && !result.specs.color) {
          result.specs.color = colorMatch[1].trim();
        }
      });
    ...
    try {
      await page.goto(`${BASE_URL}/en/search.html?q=${encodeURIComponent(productName)}`, {
      ...
      const firstResult = await page.$('.search-result-item a, .product-tile a');
      if (firstResult) {
        await firstResult.click();
        await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 30000 });
        await delay(delayMs);

        // Extract data from search result page
        const data = await page.evaluate(() => {
          const result = {
            images: [],
            description: '',
          ...
          const imgElements = document.querySelectorAll('img[src*="sika.scene7.com"]');
          const imageUrls = new Set();
          imgElements.forEach(img => {
            if (img.src) {
