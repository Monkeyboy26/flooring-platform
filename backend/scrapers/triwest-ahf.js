...
/**
 * AHF Contract enrichment scraper for Tri-West.
 *
 * Scrapes ahfcontract.com for product images, descriptions, and specs.
...
/**
 * Find a product on ahfcontract.com and extract images/specs.
 * Returns { images: string[], description: string, specs: object } or null.
 *
 * AHF Products (parent of Bruce, Hartco, Armstrong) contract site.
 * Uses Handlebars/Ractive templates with dynamic JS rendering.
 * Products rendered as .card elements with img.product src="?size=card".
 * Product sitemap: /en-us/sitemap_products.xml
 * Search: /en-us/search-results.html?searchTerm={term}
 * Detail URLs: /en-us/products/{category}/{collection}/{sku}.html
 * Image CDN: /cdn/content/sites/{siteId}/{file}?size=detail
 * Specs in table rows with th/td pairs on detail pages.
 */
async function findProductOnSite(page, productGroup, delayMs) {
  ...
  try {
    // AHF sites use search-results.html with Handlebars templates
    ...
    const detailUrl = await page.evaluate((name, sku) => {
      ...
      // First pass: exact SKU match in card text
      for (const card of cards) {
        const text = card.textContent.toLowerCase();
        const link = card.querySelector('a[href*="/en-us/"]');
        if (link && skuLower && text.includes(skuLower)) {
          return link.href;
        }
      }
      // Second pass: color name match
      for (const card of cards) {
        const text = card.textContent.toLowerCase();
        const link = card.querySelector('a[href*="/en-us/"]');
        if (link && text.includes(nameLower)) {
          return link.href;
        }
      }
      // Third pass: first product card link
  ...
  const data = await page.evaluate(() => {
    // 1. Images — AHF CDN images with ?size= param
    const images = [];
    const seen = new Set();
    ...
    // Also check for gallery/carousel images in data attributes
    ...
    // 2. Description from product detail section
    ...
    // 3. Specs from table rows (th/td or label/value pairs)
    const specs = {};
