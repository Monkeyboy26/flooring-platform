...
const BASE_URL = 'https://www.kenmarkfloors.com';
const USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const MAX_ERRORS = 30;

/**
 * Kenmark enrichment scraper for Tri-West.
 *
 * Scrapes kenmarkfloors.com for product images, descriptions, and specs.
 * Enriches EXISTING Tri-West SKUs — never creates new products.
 * Tech: Unknown (small regional brand)
 *
...
/**
 * Attempts to find product on Kenmark site.
...
async function findProductOnSite(page, group, delayMs) {
  try {
    const searchTerms = [
      group.name,
      group.collection.replace(/^Kenmark\s*/i, '').trim(),
      group.skus[0]?.variant_name
    ].filter(Boolean);

    // Strategy 1: Look for products link in navigation
    try {
      await page.goto(BASE_URL, { waitUntil: 'networkidle2', timeout: 10000 });
      await delay(1000);

      // Try to find products/collections section
      const productLinks = await page.$$eval('a[href*="product"], a[href*="collection"], a[href*="flooring"]', links =>
      ...
      // Navigation failed, try generic extraction
    }

    // Strategy 2: Generic image and text extraction from homepage
...
async function extractProductData(page, productName) {
  try {
    ...
    const description = await page.evaluate(() => {
      const selectors = ['p', '.description', '.product-description', 'article p'];
    ...
    try {
      ...
      // Extract common hardwood specs from text
      if (specText.includes('thickness') || specText.includes('Thickness')) {
