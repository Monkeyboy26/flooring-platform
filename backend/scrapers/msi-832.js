/**
 * MSI Surfaces — EDI 832 Importer (FTP)
 *
 * Connects to cftp.msisurfaces.com, downloads the latest 832 (Price/Sales Catalog)
 * file, parses EDI segments, and upserts products/SKUs/pricing/packaging
 * into the database.
 *
 * MSI provides a full price catalog via 832 EDI over plain FTP.
 * ISA Qualifier: 12, ISA Identifier: 7149990009
 *
 * Config (vendor_sources.config):
 *   ftp_host, ftp_port, ftp_user, ftp_pass
 *   isa_qualifier, isa_identifier
...
// MSI FTP has /in and /out directories
const REMOTE_DIRS = [
  '/out',
  '/in',
  '/',
...
// These must match actual slugs in the `categories` table (use sub-category slugs, not parent slugs)
const CATEGORY_MAP = {
  // ── Tile ──
  ...
  'ledger panel':           'stacked-stone',
  'ledger':                 'stacked-stone',
  'stacked stone panel':    'stacked-stone',
  // ── Luxury Vinyl ──
  ...
  'engineered wood':        'engineered-hardwood',
  'wood flooring':          'engineered-hardwood',
  ...
  'marble countertop':      'marble-countertops',
  'marble countertops':     'marble-countertops',
  'marble slab':            'marble-countertops',
  'quartzite countertop':   'quartzite-countertops',
  'quartzite countertops':  'quartzite-countertops',
  'quartzite slab':         'quartzite-countertops',
  ...
  'prefab countertop':      'prefab-countertops',
  'prefab countertops':     'prefab-countertops',
  ...
  'bullnose':               'transitions-moldings',
  'quarter round':          'transitions-moldings',
  'underlayment':           'underlayment',
  'threshold':              'transitions-moldings',
  'transition':             'transitions-moldings',
  'stair nose':             'stair-treads-nosing',
  'stair tread':            'stair-treads-nosing',
  'wall base':              'wall-base',
};
...
// Plus hypothetical long-form codes kept for future-proofing
const MAC_CATEGORY_MAP = {
  // ── Actual codes seen in MSI EDI 832 files ──
  VINTIL:  'lvp-plank',           // Vinyl Tile — LVP planks
  VIN:     'lvp-plank',           // Vinyl — longer format LVP
  INS:     'adhesives-sealants',  // Installation — grout, caulk, sealant, mortar
  INSSEA:  'adhesives-sealants',  // Installation Sealant
  BLE:     'installation-sundries', // Blended/Tools — tape, screws, levels, misc tools
  STO:     'stacked-stone',       // Stone — stacked stone panels
  STOMIS:  'natural-stone',       // Stone Miscellaneous — flagstone, ledger panels
  STOMOS:  'mosaic-tile',         // Stone Mosaic
  CER:     'porcelain-tile',      // Ceramic — ledger/wall tile
  CERFLO:  'transitions-moldings', // Ceramic Floor — bullnose, trim pieces
  WOO:     'engineered-hardwood', // Wood — engineered hardwood
  RUG:     'installation-sundries', // Rug/doormat
  // ── Long-form codes (kept for alternate EDI formats) ──
  // Tile
  ...
  WPCFLR:  'lvp-plank',
  // Hardwood
  HRDWDR:  'engineered-hardwood',
  HRDWDC:  'engineered-hardwood',
  ENGWDR:  'engineered-hardwood',
  ENGWDC:  'engineered-hardwood',
  // Countertops / Slabs
  QRTZSL:  'quartz-countertops',
  GRNSL:   'granite-countertops',
  MRBSL:   'marble-countertops',
  SLBQTZ:  'quartz-countertops',
  SLBGRN:  'granite-countertops',
  SLBMRB:  'marble-countertops',
  SLBQRZ:  'quartzite-countertops',
  QRZSL:   'quartzite-countertops',
  PRCSL:   'porcelain-slabs',
  PREFAB:  'prefab-countertops',
  // Stacked Stone
  STKSTL:  'stacked-stone',
  STKSTC:  'stacked-stone',
  LDGPNL:  'stacked-stone',
  // Outdoor
  PAVTIL:  'pavers',
  OUTDOR:  'hardscaping',
  ARTTURF: 'artificial-turf',
  // Installation & Sundries
  SETMTL:  'adhesives-sealants',
  GRTCAU:  'adhesives-sealants',
  TRMACC:  'transitions-moldings',
...
const CTP_TYPE = { RES: 'resale', NET: 'net', MSR: 'msrp', UCP: 'unit_cost', PRP: 'promotional', CON: 'contract', MAP: 'map', CAT: 'catalog', LPR: 'list_price' };

// MEA qualifier codes
const MEA_CODES = { TH: 'thickness', WD: 'width', LN: 'length', WT: 'weight', WL: 'wear_layer', HT: 'height', SQ: 'area', SU: 'surface_area', SW: 'shipping_weight' };
...
function finalizeItem(item) {
  ...
  // Product name — prefer TRN (trade name), then description
  ...
  // Collection — use PID*77 only; skip manufacturer name fallback (identifiers.mf)
  // since that yields the vendor name (e.g., "M S International Inc."), not a collection
  const collPid = item.descriptions.find(d => d.characteristic_label === 'collection');
  item.collection = collPid ? collPid.description : null;

  // Category — from GEN code, then MAC material class fallback
  ...
  // Pricing — LPR (list price) or NET/MSR fallback
  ...
  // Countertops and slabs are sold per unit
  const nameAndCat = `${item.product_name || ''} ${item.category || ''}`.toLowerCase();
  if (/countertop|slab|prefab/i.test(nameAndCat)) {
    item.sell_by = 'unit';
  }

  // Accessories/trim sold per unit
...
function getConnConfig(source) {
  ...
  return {
    host: cfg.ftp_host || process.env.MSI_FTP_HOST || DEFAULT_CONN.host,
    port: parseInt(cfg.ftp_port || process.env.MSI_FTP_PORT || DEFAULT_CONN.port, 10),
    user: cfg.ftp_user || process.env.MSI_FTP_USER || DEFAULT_CONN.user,
...
// Trim codes for accessory merging — ordered longest-first for regex alternation
const TRIM_CODES_FOR_MERGE = [
  'fsnl', 'ecl', 't-sr', '4-in-1',
  'fsn', 'osn', 'srl', 'ec', 'qr', 'sr', 'st', 'rt', 't',
];
const _trimCodeAlt = TRIM_CODES_FOR_MERGE.map(c => c.replace(/-/g, '\\-')).join('|');
const TRIM_MERGE_REGEX = new RegExp(
  `^(.+?)\\s+(${_trimCodeAlt})(-ee|-sr|-w)?\\s+[\\d.]+"?\\s*$`,
  'i'
);
...
function groupIntoProducts(items) {
  ...
  // ── Merge pass: fold trim-code accessory groups into their parent main groups ──
  // e.g., group "Aaron Ec 78"" (acc) merges into group "Aaron" (main)
  // MSI trim items often have a generic collection ("M S International Inc.") while
  // the parent main product uses the real collection name, so we also build a
  // name-only index for cross-collection fallback.
  const nameOnlyIndex = new Map(); // lowercase baseName → first matching main group
  ...
  for (const [key, group] of products) {
    ...
    const trimDesc = group.baseName.slice(parentBaseName.length).trim(); // e.g., "Ec 78""

    // Try same-collection first, then fall back to any-collection by name
    ...
    // Tag each item so the SKU loop sets variant_type/variant_name correctly
...
export async function run(pool, job, source) {
  ...
  await appendLog(pool, job.id, `Connecting to ${connConfig.host}:${connConfig.port} via FTP as ${connConfig.user}...`);
  ...
  try {
    // ── Step 1: Connect and find files (with retry) ──
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        await appendLog(pool, job.id, `FTP attempt ${attempt} failed: ${err.message}. Retrying in ${wait / 1000}s...`);
    ...
    for (const f of remoteFiles) {
      ...
      const mod = f.modifiedAt ? new Date(f.modifiedAt).toISOString().slice(0, 19) : 'unknown';
    ...
    // ── Step 2: Download ──
    localPath = `/tmp/msi_832_${Date.now()}.edi`;
  ...
  // ── Step 5b: Discontinuation detection (with safety ratio check) ──
  if (catalog.items.length >= 10) {
    ...
    // Safety: only deactivate if the imported catalog covers ≥50% of existing active SKUs
    // This prevents mass-deactivation when MSI sends a partial catalog (e.g., just tile, not vinyl)
    const matchCount = activeResult.rows.filter(r => importedSkus.has(r.internal_sku)).length;
    const ratio = activeResult.rows.length > 0 ? matchCount / activeResult.rows.length : 0;
    ...
    if (ratio >= 0.5) {
      ...
      if (deactivated > 0) {
        await appendLog(pool, job.id, `Deactivated ${deactivated} SKUs not found in latest 832 (coverage: ${(ratio * 100).toFixed(0)}%)`);
      }
    } else {
      await appendLog(pool, job.id,
        `Skipping discontinuation: only ${matchCount}/${activeResult.rows.length} (${(ratio * 100).toFixed(0)}%) active SKUs found in 832. Likely a partial catalog.`
      );
    }
  }
