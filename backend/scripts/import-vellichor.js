...
async function main() {
  ...
  for (const item of SPC_PRODUCTS) {
    ...
    // Pricing — cost is what Roma pays; retail marked up 1.65x (rounded to nickel)
    const retailPrice = +(Math.round(item.cost * 1.65 / 0.05) * 0.05).toFixed(2);
  ...
  for (const m of SPC_MOLDINGS) {
    ...
    const retailPrice = +(Math.round(m.cost * 1.65 / 0.05) * 0.05).toFixed(2);
