  ...
  for (const [collection, catKey, moldingTier, groups] of COLLECTIONS) {
    ...
    for (const [size, sqftPerBox, lbPerBox, pricePerSqft, pcsPerBox, items] of groups) {
      for (const [itemCode, colorName, species] of items) {
        ...
        // Pricing: cost = listed price, retail = cost × 1.70 (rounded to nickel)
        const cost = pricePerSqft;
        const retail = parseFloat((Math.round(cost * 1.70 / 0.05) * 0.05).toFixed(2));
        ...
        for (const [suffix, accName, accCost] of moldings) {
          ...
          // Accessory pricing: cost = listed price, retail = cost × 1.70 (rounded to nickel)
          await upsertPricing(accSku.id, accCost, Math.round(accCost * 1.70 / 0.05) * 0.05, 'per_unit');
        }
      }
    }
