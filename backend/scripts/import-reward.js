#!/usr/bin/env node
/**
 * Import the Reward Flooring catalog into the PIM.
 *
 ...
 *
 * Usage: docker compose exec api node scripts/import-reward.js
 */
import pg from 'pg';
import fs from 'fs';
...
const DATA = process.env.REWARD_DATA_DIR || path.join(__dirname, '..', 'data', 'reward');
...
function descFor(p, s) {
  const cons = s.construction === 'solid' ? 'Solid' : 'Engineered';
  const short = `${cons} ${p.species} hardwood — ${p.collection} ${p.name}${s.width ? `, ${s.width}" wide` : ''}${s.wear_layer ? `, ${s.wear_layer} wear layer` : ''}.`;
  const long = [
    `Reward Flooring ${p.collection} ${p.name} — ${cons.toLowerCase()} ${p.species} hardwood.`,
...
async function main() {
  ...
  for (const p of catalog.products) {
    ...
    const cat = (s.construction === 'solid' && catSolid) ? catSolid : catHW;
  ...
  async function trimProduct(collection) {
    ...
    const name = collection === '__generic__' ? 'Reward Trims & Moldings' : `${collection} Trims`;
    const coll = name;
