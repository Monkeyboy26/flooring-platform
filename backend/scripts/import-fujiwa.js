 ...
 *
 * Sections:
 *   1. Pool Tiles (~75 series): 6" field tiles sold by the square foot
 *      (per_sqft, no carton); mesh/small-format mosaics sold by the sheet
 *      (unit / per_unit)
...
async function main() {
  ...
  for (const [familyName, family] of Object.entries(POOL_TILE_FAMILIES)) {
    ...
    for (const [seriesCode, tileSize, sqftPerUnit, pcsPerUnit, costPerSqft] of family.variants) {
      ...
      // rate × sheet coverage). pcsPerUnit=4 → 6" field tile sold by the
      // SQUARE FOOT (sell_by=sqft / per_sqft, no carton coverage — the price
      // list has none). See fix-fujiwa-tile-mosaic-selling.mjs.
      const isSheet = family.sellByUnit || pcsPerUnit === 1;
      const sellBy = isSheet ? 'unit' : 'sqft';
      ...
      // square foot (no real carton) and accessory whole-unit items (e.g.
      // Pebblestone) carry no coverage.
      if (isSheet && !family.sellByUnit) {
