#!/usr/bin/env node
/**
 * Import the Monarch Plank catalog into the PIM.
 *
 * Monarch Plank = BRAND (code MONARCH) under NEW distributor vendor "Galleher
 * Duffy" (code GALLEHERDUFFY). Roma sources Monarch through Galleher Duffy dealer
 * account 370815. See [[monarch-onboarding]], [[vendor-sub-brands]].
 *
 * Source: backend/data/monarch/catalog.json + images.json (built by
 * build-monarch-catalog.js). Retail = cost x 1.6 nickel-rounded (keystone).
 ...
 *
 * Usage: docker compose exec api node scripts/import-monarch.js
 */
import pg from 'pg';
import fs from 'fs';
...
const DATA = process.env.MONARCH_DATA_DIR || path.join(__dirname, '..', 'data', 'monarch');
...
function descFor(p, s) {
  const dims = [s.thickness && `${s.thickness}"`, s.width && `${s.width}" wide`, s.length && `${s.length} lengths`].filter(Boolean).join(' x ').replace(' x  wide', '');
  const short = `Engineered ${p.species} hardwood — ${p.collection} ${p.name}${s.width ? `, ${s.width}" wide` : ''}${s.wear_layer ? `, ${s.wear_layer} wear layer` : ''}.`;
  const long = [
    `Monarch Plank ${p.collection} ${p.name} — engineered ${p.species} hardwood.`,
    s.thickness ? `${s.thickness}" thick${s.width ? ` x ${s.width}" wide` : ''}${s.length ? `, ${s.length} lengths` : ''}.` : '',
...
// ==================== Main ====================
async function main() {
  console.log('=== Monarch Plank Import (Galleher Duffy) ===\n');
  ...
  for (const p of catalog.products) {
    ...
    const prod = await upsertProduct(vendorId, brandId, catHW, {
    ...
    await setAttr(sku.id, 'brand', catalog.brand.name);
    await setAttr(sku.id, 'color', s.color);
    await setAttr(sku.id, 'collection', p.collection);
    await setAttr(sku.id, 'species', s.species);
    ...
    await setAttr(sku.id, 'grade', s.grade);
    await setAttr(sku.id, 'edge_type', s.edge);
  ...
  async function trimProduct(collection) {
    if (trimProdByCol.has(collection)) return trimProdByCol.get(collection);
    const name = collection === '__generic__' ? 'Monarch Trims & Moldings' : `${collection} Trims`;
    const coll = name;
    const prod = await upsertProduct(vendorId, brandId, catTrim, {
      name, collection: coll, status: 'active',
      description_short: collection === '__generic__'
        ? 'Monarch Plank transition moldings and stair parts (color-matched, sold per piece).'
        : `Color-matched moldings and stair parts for the Monarch Plank ${collection} collection.`,
      description_long: `Prefinished color-matched transitions — T-molding, reducer, end cap/threshold, stair nose, stair tread and riser — for Monarch Plank ${collection === '__generic__' ? 'engineered hardwood floors' : collection}. Sold per piece.`,
    });
    prod.is_new ? mProd++ : 0;
    trimProdByCol.set(collection, prod);
    return prod;
  }

  for (const a of catalog.accessories) {
    if (a.cost == null) { mSkipped++; continue; } // skip $0/unpriced trims
    const bucket = a.collection || '__generic__';
    const prod = await trimProduct(bucket);
    ...
    await setAttr(msku.id, 'brand', catalog.brand.name);
