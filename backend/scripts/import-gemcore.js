...
const DATA = process.env.GEMCORE_DATA_DIR || path.join(__dirname, '..', 'data', 'gemcore');
...
function descFor(p, s) {
  const kind = s.material || (p.family === 'Laminate' ? 'Waterproof Laminate' : p.family === 'SPC' ? 'SPC Rigid Core' : 'Luxury Vinyl');
  const short = `${kind} — ${p.collection} ${p.name}${s.width ? `, ${s.width}" wide` : ''}${s.wear_layer ? `, ${s.wear_layer} wear layer` : ''}. Waterproof.`;
  const long = [
    `GemCore ${p.collection} ${p.name} — ${kind.toLowerCase()}.`,
...
async function main() {
  ...
  const catLVP = await getCategoryId('lvp-plank');
  const catLam = await getCategoryId('laminate');
  const catTrim = await getCategoryId('transitions-moldings');
  if (!catLVP || !catLam) throw new Error('lvp-plank/laminate category missing');
  ...
  for (const p of catalog.products) {
    ...
    const cat = p.category === 'laminate' ? catLam : catLVP;
