#!/usr/bin/env node
/**
 * Import Cosentino — premium surfaces manufacturer (Coral Gables, FL — cosentino.com).
 *
 * Onboarded as ONE vendor (COSN) carrying 5 brands: Silestone, Dekton, Sensa, Scalea, Éclos.
 * Source: data/cosentino/catalog.json + images.json (built by build-cosentino-catalog.js from
 * scrape-cosentino.js). No price list yet → every product/SKU imported as status 'draft' with NO
 * pricing rows. Add costs + flip to 'active' when the price list arrives.
 *
 * MODEL:
 ...
 *   - Images per-color (deterministic Bynder URLs, stored remote): primary = full-slab HD render,
 *     swatch = detail thumb (alternate), lifestyle = room scenes filtered to this color's slug.
 *
 * Full rebuild: purges existing COSN products + dependents, then re-imports.
 * Usage: docker compose exec api node scripts/import-cosentino.js
 */
import pg from 'pg';
import fs from 'fs';
...
const DATA_DIR = process.env.COSN_DATA_DIR || path.join(__dirname, '..', 'data', 'cosentino');
...
function descFor(p) {
  ...
  const matLow = p.material.toLowerCase();
  const short = `${p.brand} ${p.name} — ${matLow} surface`
    + `${thicks.length ? ' (' + thicks.join(', ') + ')' : ''}. Contact for pricing & availability.`;
  const long = `${p.name} — ${p.brand} ${matLow} surface`
    + `${p.collection && p.collection !== p.brand ? ' from the ' + p.collection + ' series' : ''}, `
    + `distributed by Cosentino. ${p.description || ''}`
    + `${finishes.length ? ' Finishes: ' + finishes.join(', ') + '.' : ''}`
    + `${thicks.length ? ' Thicknesses: ' + thicks.join(', ') + '.' : ''}`
    + `${p.skus[0] && p.skus[0].size_nominal ? ' Slab format: ' + p.skus[0].size_nominal + '.' : ''}`;
...
async function importProduct(p, vendorId, brandId, catId) {
  ...
  // Per-color images at product level (all SKUs share them).
  const im = images[p.pkey];
  let hasImg = false;
  if (im) {
    if (im.primary) { await productMedia(productId, im.primary, 'primary', 0); hasImg = true; }
    if (im.swatch) await productMedia(productId, im.swatch, 'alternate', 0);
    let so = 0;
  ...
  for (const s of p.skus) {
    ...
    const vendorSku = internal.replace(/^COSN-/, '');
    ...
    // NO upsertPricing — no price list yet (draft).
  ...
  return { skuN: p.skus.length, hasImg };
}

async function main() {
  console.log('=== Cosentino Import ===\n');
  const vendorId = await upsertVendor(catalog.vendor);
  console.log(`Vendor: ${catalog.vendor.name} (${vendorId})`);

  // Brands (5) — resolve id per brand code for product assignment.
  const brandIdByCode = {};
  for (const b of catalog.brands) {
    const bid = await upsertBrand(b);
    await linkVendorBrand(vendorId, bid, false);
    brandIdByCode[b.code] = bid;
    console.log(`Brand:  ${b.name} (${bid})`);
  }

  const CAT = {};
  for (const s of new Set(catalog.products.map((p) => p.category))) CAT[s] = await getCategoryId(s);

  // Full rebuild: purge existing Cosentino products + dependents
  ...
  if (prodIds.length) {
    ...
    console.log(`\nPurged ${prodIds.length} existing Cosentino products (${skuIds.length} SKUs)\n`);
  }

  let pN = 0, sN = 0, imgN = 0;
  const byBrand = {};
  for (const p of catalog.products) {
    const { skuN, hasImg } = await importProduct(p, vendorId, brandIdByCode[p.brandCode], CAT[p.category]);
    pN++; sN += skuN; if (hasImg) imgN++;
  ...
  console.log('\nDone. Products are DRAFT (not on storefront) until pricing is added + status flipped to active.');
  await pool.end();
}
