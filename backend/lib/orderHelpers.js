...
// Roll shipment quantities up to the order: recompute orders.fulfillment_status
// (unfulfilled | partial | fulfilled) and, for shipping orders, auto-advance
// orders.status along the fulfillment axis (confirmed -> processing -> shipped
// -> delivered). Pure DB only — the CALLER fires side-effects (invoice/email/
// commission/tracking) based on the returned transition, since those live in
// server.js. Never touches cancelled/refunded orders. Pickup orders get their
// fulfillment_status updated but keep the manual ready_for_pickup/delivered flow.
export async function recalcFulfillment(queryable, orderId) {
  const db = queryable;
  const ordRes = await db.query(
    'SELECT id, status, delivery_method, shipped_at, delivered_at FROM orders WHERE id = $1',
    [orderId]
  );
  if (!ordRes.rows.length) return null;
  const order = ordRes.rows[0];
  const previousStatus = order.status;

  // Ordered qty per non-sample line item.
  const itemsRes = await db.query(
    'SELECT id, num_boxes FROM order_items WHERE order_id = $1 AND COALESCE(is_sample, false) = false',
    [orderId]
  );
  // Shipped qty per item — only shipments that have actually shipped/delivered
  // count toward fulfillment (a 'packing' shipment allocates boxes but hasn't
  // shipped yet, so it must NOT mark the line as fulfilled).
  const shipItemsRes = await db.query(
    `SELECT si.order_item_id, COALESCE(SUM(si.qty_boxes), 0)::int AS shipped
       FROM shipment_items si
       JOIN shipments s ON s.id = si.shipment_id
      WHERE s.order_id = $1 AND s.status IN ('shipped', 'delivered')
      GROUP BY si.order_item_id`,
    [orderId]
  );
  const shippedByItem = {};
  for (const r of shipItemsRes.rows) shippedByItem[r.order_item_id] = r.shipped;

  let totalShipped = 0;
  let allFullyShipped = itemsRes.rows.length > 0;
  for (const it of itemsRes.rows) {
    const ordered = it.num_boxes || 0;
    const shipped = shippedByItem[it.id] || 0;
    totalShipped += Math.min(shipped, ordered);
    if (shipped < ordered) allFullyShipped = false;
  }

  let fulfillment_status = 'unfulfilled';
  if (itemsRes.rows.length && totalShipped > 0) {
    fulfillment_status = allFullyShipped ? 'fulfilled' : 'partial';
  }

  // Active (non-cancelled) shipment states drive the order-status rollup.
  const shipRes = await db.query(
    "SELECT status, shipped_at, delivered_at FROM shipments WHERE order_id = $1 AND status <> 'cancelled'",
    [orderId]
  );
  const active = shipRes.rows;
  const anyShipped = active.some(s => s.status === 'shipped' || s.status === 'delivered');
  const allDelivered = active.length > 0 && active.every(s => s.status === 'delivered');

  let newStatus = order.status;
  const terminal = order.status === 'cancelled' || order.status === 'refunded';
  if (!terminal && order.delivery_method === 'shipping') {
    if (fulfillment_status === 'fulfilled' && allDelivered) newStatus = 'delivered';
    else if (fulfillment_status === 'fulfilled' && anyShipped) newStatus = 'shipped';
    else if (active.length > 0) newStatus = 'processing';
    else if (['processing', 'shipped', 'delivered'].includes(order.status)) newStatus = 'confirmed';
  }

  // Timestamps track the rolled-up status.
  const shippedTimes = active.map(s => s.shipped_at).filter(Boolean).sort();
  const deliveredTimes = active.map(s => s.delivered_at).filter(Boolean).sort();
  const earliestShipped = shippedTimes.length ? shippedTimes[0] : null;
  const latestDelivered = deliveredTimes.length ? deliveredTimes[deliveredTimes.length - 1] : null;

  let shippedAt = order.shipped_at;
  let deliveredAt = order.delivered_at;
  if (['shipped', 'delivered'].includes(newStatus)) {
    if (!shippedAt) shippedAt = earliestShipped || new Date();
  } else {
    shippedAt = null;
  }
  if (newStatus === 'delivered') {
    if (!deliveredAt) deliveredAt = latestDelivered || new Date();
  } else {
    deliveredAt = null;
  }

  await db.query(
    'UPDATE orders SET fulfillment_status = $1, status = $2, shipped_at = $3, delivered_at = $4 WHERE id = $5',
    [fulfillment_status, newStatus, shippedAt, deliveredAt, orderId]
  );
  ...
  return {
    ...
    transitionedTo: newStatus !== previousStatus ? newStatus : null,
  };
}
