...
async function main() {
  ...
  for (const [collection, products] of byCollection) {
    ...
    for (const prod of products) {
      ...
      // Strategy 4: For products where name contains install type (Glue Down, Rigid Click),
      ...
      // Strategy 5: For products with no color and only one Coveo non-trim color, use it
