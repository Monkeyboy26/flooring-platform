                  ...
                  <div>
                    ...
                    {wlSelected.length > 0 && (
                      <div style={{
                        ...
                        padding: '0.875rem 1.375rem', marginBottom: '1.5rem', borderRadius: 6,
                    ...
                    {wishlistSkus.length === 0 ? (
                      ...
                      <div className="acct-wishlist-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: '1rem' }}>
                        {wishlistSkus.map(sku => {
                          ...
                          return (
                            <div key={sku.sku_id} className="acct-wishlist-card"
                              ...
                              <div style={{ position: 'relative', overflow: 'hidden', cursor: 'pointer' }}
                                ...
                                <button aria-label="Remove from wishlist" title="Remove from wishlist"
                                  onClick={e => { e.stopPropagation(); toggleWishlist(sku.sku_id); }}
                                  style={{
                                    position: 'absolute', top: 10, right: 10, width: 24, height: 24, borderRadius: '50%',
                                    background: 'rgba(255,255,255,0.92)', border: 'none',
                                    color: 'var(--stone-600)', fontSize: '0.875rem', lineHeight: 1, cursor: 'pointer',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0,
                                    boxShadow: '0 1px 4px rgba(28,25,23,0.15)'
                                  }}>&times;</button>
                              </div>
                              <div className="acct-wishlist-card-body" style={{ gap: '0.375rem' }}>
                                {sku.collection && <div className="acct-footer-card-sub">{sku.collection}</div>}
                                <div onClick={() => onSkuClick(sku.sku_id, sku.product_name || sku.collection)}
                                ...
                                {price != null && (
                                  <div style={{ fontSize: '0.8125rem', color: 'var(--stone-600)' }}>
                                    <strong style={{ color: 'var(--stone-800)', fontWeight: 500 }}>${displayPrice(sku, price).toFixed(2)}{priceSuffix(sku)}</strong>
                                    {meta && <span style={{ fontSize: '0.75rem' }}> &middot; {meta}</span>}
                                  </div>
                                )}
                                <div style={{ display: 'flex', gap: '0.375rem', marginTop: '0.5rem' }}>
                                  <button className="acct-btn acct-btn--outline" style={smallBtn} onClick={() => orderSamples([sku])}>Sample</button>
                                  <button className="acct-btn" style={smallBtn} onClick={() => onSkuClick(sku.sku_id, sku.product_name || sku.collection)}>View →</button>
                                </div>
                              </div>
                            </div>
                          );
      ...
      return (
        <label style={{ display: 'grid', gap: 7 }}>
          ...
          {textarea ? (
            ...
            <div style={{ position: 'relative' }}>
              <input type={effType} value={value} onChange={onChange} onBlur={onBlur} placeholder={placeholder} maxLength={maxLength} autoComplete={autoComplete} style={base}
                onFocus={e => e.target.style.borderBottomColor = accent} onBlurCapture={e => { e.target.style.borderBottomColor = lineColor; }} />
              {isPw ? (
                <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => setReveal(r => !r)} aria-label={reveal ? 'Hide password' : 'Show password'} style={{ position: 'absolute', right: 0, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: muted, font: '500 10px/1 ui-monospace, monospace', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '4px 0' }}>{reveal ? 'Hide' : 'Show'}</button>
    ...
    // ==================== About Page (Direction A — "The Letter") ====================
    ...
    function AboutImgSlot({ label, ratio = '4 / 3' }) {
      ...
      return (
        <div style={{
          aspectRatio: ratio, width: '100%', background: '#ddd4c1',
          border: `1px solid ${ink}20`, position: 'relative',
          display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
        }}>
          <div style={{ position: 'absolute', inset: 10, border: `1px dashed ${ink}26`, pointerEvents: 'none' }} />
          <div style={{ textAlign: 'center', padding: 20 }}>
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={ink + '66'} strokeWidth="1.2" style={{ marginBottom: 10 }} aria-hidden="true">
              <rect x="3" y="3" width="18" height="18" rx="1" />
              <circle cx="8.5" cy="8.5" r="1.6" />
              <path d="M21 15l-5-5L5 21" />
            </svg>
            <div style={{ font: '500 10px/1.5 var(--font-body)', letterSpacing: '0.18em', textTransform: 'uppercase', color: ink + '88' }}>{label}</div>
          </div>
        </div>
      );
    }
